...
typedef void(^RestorePurchasesCompletionBlock)(NSError *error);
typedef void(^RefreshReceiptCompletionBlock)(NSError *error);
...
- (void)getProductsForIds:(NSArray *)productIds completion:(ProductsCompletionBlock)completionBlock error:(ErrorBlock)err;
